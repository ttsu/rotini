...
## Enhancement A — Member availability / unavailability

The round-robin must skip members who are away on a given occurrence's date. Today it walks `rota_members` by `position`, filtering out viewers; this adds a date-aware skip.

### 31. Migration — `member_unavailability` + availability-aware materializer + RPCs

- New migration `…_member_unavailability.sql`:
  - Table:
    ```
    member_unavailability
      ...
      start_date  date          -- inclusive, in rota.tz
      end_date    date          -- inclusive
      reason      text null
      created_at  timestamptz
      CHECK (end_date >= start_date)
      INDEX (rota_id, user_id, start_date, end_date)
    ```
  - Two `SECURITY DEFINER` RPCs:
    - `set_unavailability(rota_id, start_date, end_date, reason)` — caller must be a non-viewer member of the rota; upserts a window; cannot overlap an existing window for the same user (merge or reject — reject with a clear error in v1).
    - `clear_unavailability(unavailability_id)` — caller must own the row (or be a rota owner).
  - RLS: a member manages **their own** rows; all rota members (incl. viewers) can **read** the rota's windows (so the coordinator and peers can see who's away). Owners may delete any row in their rota.
- Materializer change (`materialize-rota` DB function + edge function): when walking the round-robin cursor over non-viewer members, **also skip any member whose unavailability window contains the occurrence's `scheduled_local_date`**. The cursor still advances to the next eligible member.
  - **All members unavailable for a date** → write the occurrence with `assigned_user_id = NULL` and `status = 'open'` (see Enhancement B for the `open` status + nullable assignee). Do **not** crash or leave a gap in the schedule.
  - Setting/clearing unavailability **regenerates only future, non-overridden, non-`done` occurrences** for that rota (same rule as a recurrence change). Past, overridden, and completed rows are preserved.
- Reminder reconcile: regenerating occurrences must re-run the `enqueue-notifications` step so jobs for reassigned turns are cancelled/re-created (Phase 5 machinery).
- Re-run `npm run db:types`.

### 32. Availability UI

- **Member self-service:** on the rota detail screen (and/or Settings-for-this-rota), a "I'm away…" control → date-range picker + optional reason → `set_unavailability`. A list of the member's own upcoming windows with delete.
- **Coordinator visibility:** on the roster/members view, show an "away" indicator next to members with a window overlapping the next N days, and a compact "who's away" list. Owners can clear any window.
- **Schedule feedback:** after saving, the upcoming list/calendar visibly re-flows (occurrences reassigned around the away window); any date with no eligible member shows as an **Open** turn (ties into Enhancement B).
- Realtime: subscribe to `member_unavailability` for the current rota so other members' away windows and the resulting reassignments land live.
...
**Enhancement A — availability**

- Member marks an away window → future occurrences on those dates reassign to the next eligible member; the cursor advances correctly; past/overridden/done rows untouched.
- Window covering a date where **every** member is away → that occurrence becomes `open` (null assignee), not a crash or a gap.
- Clearing a window re-materializes and restores normal rotation for future dates.
...
- `pgTAP` for `set_unavailability` / `clear_unavailability` permission + overlap paths, the materializer skip logic, `claim_coverage` race (two concurrent claims → one success), and `get_shared_rota` anon-reachability + revoked-token rejection.

## Done-when

- [ ] **A:** availability table + RPCs + RLS; materializer skips away members and produces `open` turns when nobody's eligible; reminders reconcile; UI lets members set/clear windows and coordinators see who's away. Realtime live.
